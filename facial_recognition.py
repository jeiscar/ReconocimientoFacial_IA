...
# CONFIG
path = "C:/facial_recognition-main/img" # your path
txt_login = "Toma asistencia"
...
def credentials(screen, var, flag):
    ...
    Label(screen, text="Estudiante:", fg=color_white, bg=color_background, font=(font_label, 12)).pack()
...
def login_capture():
    ...
    img = f"\\img\\{user_login}_login.jpg"
    img_user = f"\\img\\{user_login}.jpg"
    ...
    if(res_db["affected"]):
        ...
        if img_user in my_files:
            ...
            printAndShow(screen2, "¡Error! Estudiante no encontrado", 0)
    else:
        printAndShow(screen2, "¡Error! Estudiante no encontrado", 0)
    os.remove(img)
